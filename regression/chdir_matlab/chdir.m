...
if any(ctrlConstantGenes)
    err3 = MException('ConstantCheck:HasConstantRows',...
        sprintf('%s row(s) in control expression data are constant. Consider Removing the row(s).', strjoin(num2str(find(ctrlConstantGenes)),' ')));
    throw(err3);
elseif any(expmConstantGenes)
    err3 = MException('ConstantCheck:HasConstantRows',...
        sprintf('%s row(s) in experiment expression data are constant. Consider Removing the row(s).', strjoin(num2str(find(expmConstantGenes)),' ')));
    throw(err3);
end
